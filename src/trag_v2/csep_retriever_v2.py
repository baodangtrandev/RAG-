...
class CSEPRetrieverV2:
    ...
    def __init__(self, retriever=None, llm_generate_fn=None, top_k_retrieve: int = None,
                 smart_hop2: bool = True, hop1_dist_threshold: float = 0.55, csep: bool = True):
        ...
        logger.info(f"[CSEP v2] Init: smart_hop2={self.smart_hop2}, threshold={self.hop1_dist_threshold}, csep={self.csep}")
    ...
    def retrieve_batch(self, queries: List[str]) -> List[List[Dict[str, Any]]]:
        ...
        # ============================================================
        # Sub-stage B: Xác định các query thực sự cần chạy Hop 2 (Smart Hop 2)
        # ============================================================
        csep_candidate_flags = [True] * n
        for i in range(n):
            if self.smart_hop2:
                # 1. Kiểm tra số lượng active shards
                ...
                # 2. Kiểm tra chất lượng Hop 1 (top-1 distance)
                docs = hop1_results[i]
                if docs:
                    top_distance = docs[0].get("vector_distance", 1.0)
                    if top_distance < self.hop1_dist_threshold:
                        # Match chất lượng cao, skip Hop 2 để tiết kiệm latency
                        csep_candidate_flags[i] = False
                        continue
        ...
        # ============================================================
        # Sub-stage C: Batch Entity Extraction cho các query được chọn
        # ============================================================
        ...
        if hop2_queries:
            ...
            with ThreadPoolExecutor(max_workers=self.retrieve_workers) as executor:
                active_hop2_results = list(executor.map(
                    # Trong Hop 2, ta chỉ lấy các docs mà không cần router/emb lại
