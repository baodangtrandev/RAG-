...
review progressed through the four PRISMA flow stages —
identification, selection, eligibility, and inclusion — while the
...
lished between 2020 and May 2025. 1 This citation-based filter
...
1Citation statistics were collected from Semantic Scholar on 13 May 2025.
arXiv:2508.06401v3 [cs.DL] 9 Sep 2025
...
highly cited RAG studies?
Summarises the main topics in the
...
generation systems?
By identifying relevant metrics, re-
...
techniques?
Identifies research gaps, enabling
...
A. Systematic Review Framework Selection
...
initial screening and a review of the full text.
1) Initial Screening:After removing duplicates and apply-
...
4
available in a publicly accessible Google Sheets workbook 2
...
2RAG_Data_Extraction.xlsx
tion, which includes details such as the number of questions;
...
6
TABLE III: Study characteristics of 128 included RAG papers by domain: datasets, chunking mechanisms, retrieval mechanisms, vector-space encoders,
...
retrieval/encoding. [58]
•Asynchronous Updates - Re-
...
•The Stack [81]
•VerilogEval [85]
•VerilogEval-syntax [85]
•20 lines per chunk [81]
...
Research Society’s, UpToDate [93]
•1000 tokens with an overlap
of 100 tokens [88]
...
•ReCoRD [110]
•WikiEval [114]
•Wizards of Wikipedia [110]
•Fixed-size sliding windows
of 300 words per chunk
[108]
•Dense Retrieval [108],
[109], [111]
•Contriever (Dense) [108]
•Dragon (Dense) [108]
•Ask Practical Law AI proprietary
LLM [111]
•Claude-3-Opus [109]
...
•PubMed Clinical Papers [135]
•Extracts multiple contextual
...
mary/Q&A chunk [143]
•Dense Retrieval [143], [144]•BGE (Dense) [143]
•SGPT (Dense) [143]
...
[28], [122]. Research has extended these core indexing meth-
ods to distributed and dynamic environments, employing GPU-
...
inputs to continuous embeddings that capture contextual and
semantic nuances:
...
studies have extended RAG to specialised and multimodal
tasks. In biomedicine, BioGPT applied retrieval-augmented
...
of interlinked pages as a long “retrieval unit” (≈4k tokens).
This 30-fold reduction in retrieval units (for example, from 22
...
dynamically inserts near-neighbour exemplars while assem-
bling the prompt. Systems such as R-GQA and MolReGPT
...
livering a five-point F 1 improvement over nearest-neighbour
...
applies a text encoder to retrieve the precise passages required
for answer generation [63].
...
LLM “graph-ready” without bespoke retraining. As modalities
proliferate—tables, time series, 3-D scenes—the foundational
...
thought, sentence, or document at a time—prevents prompt
lengths from exploding and keeps provenance transparent.
...
for retriever and generator use, boosting F 1 by over eight
...
storeK E timestamps canonicalised entities from browsing
...
LiVersa encrypts clinical memories at rest andK E avoids raw
...
architecture goes further, treating each memory as an explicit
action token that the agent can revisit when planning complex
...
possible, but dependable.
10) Synthesis & Outlook:The evidence in this review indi-
...
F1 scoreis the harmonic mean of the precision and recall
of the token level:
...
wn logp n

wherep n is the n-gram precision fornup to typically 4 [37],
...
kX
i=1
P(i)1{doc i is relevant},
whereN q is the number of relevant documents for q andP(i)
...
log2(i+1)
IDCG@k ,
whererel i is the relevance grade of theith document and
...
sures often obscure which individual components drive gains
and presuppose the availability of strong baselines in the target
domain.
...
selves as automated judges of generated content. Rather than
relying solely on surface-level overlap or costly human annota-
...
marks, although diverse in their origins and target domains,
collectively map out a multidimensional landscape of model
...
MARCO [176] provide a baseline, particularly for question-
answer tasks. These datasets excel in benchmarking fluency
...
Hybrid pipelines that anchor language-agnostic schemas with
thin domain-specific rules travel better, but add engineering
...
overtly toxic text, highlighting how difficult covert bias de-
tection will be once adversaries understand retrieval scoring.
...
although borderline cases may remain.
Screening and extraction procedures also introduce potential
...
(MuSiQue) [189]
25,000 2–4-hop questions (50,000 with contrast). Multi-hop QA by composing single-hop questions. 9
...
[176]
100,000 questions and 1 M passages from web docs. Reading comprehension and QA from real web data. 8
...
(ASQA) [201]
12,632 ambiguous QA annotations. Long-form QA for ambiguous factoid questions. 4
...
Evaluation) (CodeXGLUE) [219]
Millions of code - NL pairs across tasks. Code understanding and generation. 2
...
(IIRC) [229]
13,441 questions, 5,698 paragraphs. Challenging reading comprehension. 2
...
[232]
14k visual questions needing external knowledge. Visual QA with outside knowledge. 2
...
tences.
Dense video event description modeling. 1
...
actions.
Retrieval-based memory for safe driving planning. 1
Aggregated flood event listings from EMSR, GDACS, and
ReliefWeb [100]
Curated list of major global flood disasters. Provide event codes for UI. 1
...
AIDA CoNLL-Y AGO [255] CoNLL03 news articles linked to Y AGO entities. Named entity disambiguation tasks. 1
...
embeddings.
Drive precise biomedical RAG for Alzheimer’s queries. 1
...
[261]
30 liver disease clinical practice guidelines Reference for hepatology QA tasks 1
...
[53]
QA pairs with correct BioCypher graph queries. Evaluate LLM-to-KG query translation accuracy. 1
...
[270]
2020 global population grid disaggregated by census. Quantify populations in flood zones. 1
...
of biomedical relation extraction algorithms
1
...
[281]
301k English/German tokens for NER. Named-entity recognition benchmark. 1
Conference on Natural Language Learning 2004 (CoNLL04)
[282]
2k sentences for NER and SRL. Joint NER and semantic-role labeling. 1
...
[137]
62,886 abstract - lay summary pairs from biomedical journals. Simplify scientific text. 1
...
articles.
Test LLM’s ability to learn new facts via fine-tuning/RAG. 1
...
(EASL) [308]
HCV screening, diagnosis, and treatment guidelines. Hepatology clinical decision support. 1
...
tured information (FEVEROUS) [311]
87,026 claims with text and table evidence. Automate claim verification using text/tables. 1
FAct Verification from Information-seeking Questions (FaV-
IQAmbig) [312]
188,000 true/false claims from info-seeking queries. Generate and assess factual QA claims. 1
...
ams, doctrine).
Benchmark legal-AI retrieval for practicing attorneys. 1
...
[349]
11.5k college-level multimodal exam questions. Expert-level multimodal reasoning evaluation. 1
...
and support future optimisation of the chatbot service.
1
...
(ReCoRD) [403]
70 k passages, 120 k queries Commonsense reading comprehension 1
...
(WaNLI) [441]
107,885 NLI examples combining human and GPT-3 data. Natural language inference with AI mix. 1
...
The authors appreciate the use of the Kelvin 2 High Perfor-
...
advs.202306724
[13] B. Zhang and H. Soh, “Extract, define, canonicalize: An
...
Language Technologies: Student Research Workshop, 2022, Conference
...
2023, Conference Proceedings, pp. 1720–1736. [Online]. Available:
https://www.scopus.com/inward/record.uri?eid=2-s2.0-85184807859&
...
2024. [Online]. Available: https://www.scopus.com/inward/record.
uri?eid=2-s2.0-85191075594&doi=10.1038%2fs41746-024-01091-y&
partnerID=40&md5=6d44109de8d3a3997cf2069604d69bedhttps:
...
Conference Proceedings, pp. 8460–8478. [Online]. Available:
https://www.scopus.com/inward/record.uri?eid=2-s2.0-85136224764&
...
language models,” p. arXiv:2304.10592, April 01, 2023 2023. [Online].
...
T. Gogineni, G. Goh, R. Gontijo-Lopes, J. Gordon, M. Grafstein,
S. Gray, R. Greene, J. Gross, S. S. Gu, Y . Guo, C. Hallacy,
...
Conference Proceedings, pp. 1533–1544. [Online]. Available:
...
Computational Linguistics, 2019, Conference Proceedings, pp. 4149–
4158. [Online]. Available: https://aclanthology.org/N19-1421/https:
...
[252] J. Mao, J. Ye, Y . Qian, M. Pavone, and Y . Wang, “A language
...
(CoNLL-2004) at HLT-NAACL 2004. Association for Computational
Linguistics, 2004, Conference Proceedings, pp. 1–8. [Online].
...
M. Shoeybi, and B. Catanzaro, “Factuality enhanced language
models for open-ended text generation,” pp. 34 586–34 599, 2022.
...
of the North American Chapter of the Association for Computational
Linguistics: Human Language Technologies. Association for
Computational Linguistics, 2021, Conference Proceedings, pp. 2523–
...
2018, Conference Proceedings, pp. 252–262. [Online]. Available: https:
...
[403] S. Zhang, X. Liu, J. Liu, J. Gao, K. Duh, and
