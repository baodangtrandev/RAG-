...
RAG Applications
Cheng Chang 1, Jingwei Ge 1, Jiazhe Guo 2, Zelin Guo 1, Binghong Jiang 1, Li Li 1*
...
First, conventional scenario embedding
...
posed some scenario embedding models via
...
rear, rear left, rear right} (2)
cr
i = α |N r
i | , α V 2V /αV 2N = 1/4 (3)
...
of each attention head are Qi ∈ Rdq , Ki ∈
...
.
.
Heterogeneous Vehicle Interactions Multi-Expert Scenario Sets HNSW with All Scenario Data
...
...
...
PromptNearest
...
• High-Density Scenarios : For scenarios where p(si) > Quantileα%(p(S)), sample them with
probability inversely proportional to their density: P (select(si)) = 1
p(si) with β%:
...
Steps:
1. T op-K Similar Scenarios Arrangements:
• Categorize all top- K scenarios for n prompts
into K levels as T = {T 1, T 2, . . . , TK}, where
T l = {tl
i | tl
...
ries are generated using 5 th-degree polynomial in
...
Intersection-EP. With the scenario database,
...
T able 1: Comparison of different embedding dimensions in scenario datasets.
DataSet INTERACTION: Intersection Scenario CitySim: FreeWayC Scenario 
Embedding 
Dimension 
Distance with 
Prompt Scenarios⭣ 
...
15~19 
1.79 1.22 0.83 
32 3.67 2.58 0.89 2.07 1.23 0.86 
64 2.84 2.32 0.91 1.78 1.09 0.88 
128 3.34 2.38 0.94 1.73 1.15 0.88 
256 2.99 2.34 0.95 1.95 1.29 0.89 
Graph-DTW MDS 1.89 1.99 \ >960 0.71 0.70 \ 
...
W orld): HNSW builds a multi-level graph where
...
only 3 ms to complete the search task in typical
...
HNSW32 base method. As the α parameter
...
T able 2: Comparison of different parameters ( α, β) for HNSW32-TSD searching performance.
Parameter (𝜶, 𝜷) 
Search 
Time⭣ 
Search 
...
Search 
Distance⭣ 
(90%, 5%) 5.60 ms 3.00 (90%, 5%) 5.60 ms 3.00 
(80%, 5%) 6.50 ms 2.99 (90%, 10%) 6.67 ms 2.97 
(60%, 5%) 8.26 ms 2.93 (90%, 20%) 7.72 ms 2.96 
(30%, 5%) 10.11ms 2.90 (90%, 30%) 8.71 ms 2.94 
...
accumulated scenarios reaches the scale of 10 5,
...
T able 3: Comparison of LLM-RAG planning results with different conditions in different scenarios.
Dataset INTERACTION: Intersection Scenario CitySim: FreeWayC Scenario 
Metric 
RAG Type 
Human-Likeness 
...
Goal 
Confusion⭣ 
Polynomial-Based 2.43 11.1% 14.8% 18.5% 2.27 6.1% 8.2% 12.2% 
LLM w/o RAG 2.02 4.8% 8.5% 8.5% 4.94 4.9% 3.0% 10.9% 
RAG-Random 1.80 4.8% 6.2% 7.4% 2.28 4.0% 3.0% 1.9% 
RAG-All 1.58 2.6% 4.8% 6.2% 1.83 1.9% 0.0% 1.9% 
RAG-TSD 1.63 2.6% 3.7% 7.4% 1.85 1.0% 1.9% 1.9% 
RAG-All-RO 1.47 1.1% 2.6% 4.9% 1.74 1.9% 0.0% 0.0% 
RAG-TSD-RO 1.50 2.6% 3.7% 3.7% 1.76 1.9% 1.0% 0.0% 
 
in Table 3, we adopt the average displacement
error (ADE) between LLM planning trajectory
with human driving trajectory in future 3 s time
...
(a) Original Scenario and Trajectory (b) Planning Trajectory w/o RAG
(c) Planning Trajectory by RAG-TSD (d) Planning Trajectory by RAG-TSD-RO
Motion 
Flow
...
to about 30 ms at the 10 5 level of data scale via
...
[12] Asam OpenSCENARIO Domain-
Specific Language, 2.1.0. Online (2024).
https://www.asam.net/standards/detail/
openscenario-dsl/
[13] Althoff, M., Koschi, M., Manzinger, S.:
...
ulation: Interpretable decision-making
for autonomous vehicles with retrieval-
...
models. arXiv preprint arXiv:2406.04300
(2024)
...
robust autonomous perception and sce-
...
using hierarchical navigable small world
...
Konigshof, H., Stiller, C., La Fortelle,
...
in Engineering Software 41(4), 548–556
(2010)
...
Chandraker, M.: Llm-assist: Enhancing
closed-loop planning with language-based
