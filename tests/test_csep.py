...
def test_extract_entities_batch_calls_llm_with_correct_count():
    ...
    mock_llm = MagicMock(return_value=[json.dumps({"entities": ["JIRA-123"]}), json.dumps({"entities": ["PR-456"]})])
    retriever = _make_mock_retriever()
...
def test_retrieve_batch_csep_off_returns_hop1_only():
    ...
    retriever = _make_mock_retriever(probs=[0.9, 0.05, 0.01, 0.01])  # chi slack > 0.15
    mock_llm = MagicMock(return_value=[json.dumps({"entities": []})])
...
def test_retrieve_batch_csep_on_augments_query():
    ...
    retriever = _make_mock_retriever(probs=[0.9, 0.8, 0.01, 0.01])  # slack + jira > tau -> CSEP=True
    mock_llm = MagicMock(return_value=[json.dumps({"entities": ["JIRA-999"]})])
    csep = _make_csep(enable_for_all=True, retriever=retriever, llm_fn=mock_llm)
