...
MARKDOWN_SPLITTER = MarkdownHeaderTextSplitter(
    ...
    strip_headers=False  # Retain headers in the resulting chunks for context
)
...
def process_row(row_dict: dict) -> list[dict]:
    ...
    try:
        ...
        logger.warning(f"Markdown splitting failed for doc {original_doc_id}: {e}. Falling back to recursive splitting.")
...
def main(input_file: str, output_file: str, max_workers: int):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        results = list(tqdm(
            executor.map(process_row, rows, chunksize=100), 
            total=len(rows), 
            desc="Chunking Progress"
        ))
